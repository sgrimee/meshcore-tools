...
def test_migrate_settings_toml_to_config(tmp_path):
    """On first load, non-secret data from settings.toml is migrated to config.toml."""
    (tmp_path / "settings.toml").write_text('[general]\nregion = "MIG"\n')
    result = load_config(tmp_path)
    assert result.get("general", {}).get("region") == "MIG"
    assert (tmp_path / "config.toml").exists()


def test_migrate_strips_default_password(tmp_path):
    """default_password at top level must NOT be copied into config.toml during migration."""
    # In the old format, default_password was a top-level key in settings.toml.
    (tmp_path / "settings.toml").write_text('default_password = "secret"\n\n[general]\nregion = "MIG"\n')
    load_config(tmp_path)
    import tomllib
    data = tomllib.loads((tmp_path / "config.toml").read_text())
    assert "default_password" not in data
    assert data.get("general", {}).get("region") == "MIG"


def test_aot_round_trip(tmp_path):
    """Lists of dicts are serialized as TOML arrays-of-tables and round-trip correctly."""
    data = {
        "connection": {
            "type": "tcp",
            "history": [
                {"type": "tcp", "host": "1.2.3.4", "port": 5000},
    ...
    assert loaded["connection"]["type"] == "tcp"
